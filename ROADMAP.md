...
**Progress (2026-03-02):** 11 of 16 deliverables complete across 4 commits (`e2c68af` through `a6586de`). Reader experience is now immersive: dwell contemplation (long-press/hover), keyboard navigation (j/k/arrows), lotus bookmarks with `/bookmarks` page, drop caps and epigraph typography, reading time + scroll indicator, focus/presentation modes, parting words, passage page polish, SW chapter caching, speculation rules. 306 Vitest tests across 22 files (all passing). Performance: all pages ≤ 128KB First Load JS (budget raised from 120→128KB — justified by reader interactivity: bookmarks, keyboard nav, scroll indicator add ~4KB page-specific JS). Remaining: M2b-7 (Contextual Quiet Corner), M2b-12 (HyDE), M2b-13 (Cohere Rerank), M2b-14 (Vercel KV — likely skip), M2b-16 (Low-bandwidth extended).
...
| M2b-16 | **Low-bandwidth detection — extended adaptation** | Foundation established in M1c-15 (2G/slow-2G banner with text-only mode suggestion). 2b extends: adaptive image quality reduction on slow connections (serve lower-resolution book covers, compress OG images), adaptive debounce extension on search suggestions, `localStorage` persistence upgrade (1c uses `sessionStorage`). Complements M1c-15 — seekers already get the core protection from first deployment. (ADR-006) |
