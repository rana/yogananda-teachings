## DES-063: Book Reader — The Reading Surface
...
**Supersedes:** Reading-related sections scattered across DES-006 (DES-008, DES-009, DES-011, DES-012, DES-013), reading mode definitions in ADR-006 and ADR-072
**Related:** DES-025 (Accessibility), DES-049 (Responsive), DES-054 (Knowledge Graph), ADR-085 (Book Ingestion)
...
**Future:** `data-mode="cast"` from DES-049 becomes unnecessary. Immerse *is* the cast mode at wider viewports.
...
**Hindi reading:** Noto Serif Devanagari at 20px / 1.9 line height (ADR-080). Hindi body text is larger than Latin because Devanagari requires more vertical space for legibility at equivalent perceived size.
...
**"Breath Between Chapters" (DES-012):** When navigating via these links (not deep links or search), a 1.2-second transitional moment shows only the chapter title (Lora 400, centered, fading in). Then the chapter text fades in over 400ms. Skipped for `prefers-reduced-motion`, deep links, and search result arrivals.
...
Print is a first-class reading mode. Seekers print passages — this is an act of devotion, not edge-case usage (DES-025).
...
- **ADR-006** § 5 (Presentation mode) — absorbed into Immerse (section 2.1)
- **ADR-072** § 3 (Simplified reading mode / Focus) — absorbed into Immerse (section 2.1)
- **DES-006** § DES-009 (Dwell) — replaced by Zoom Paragraph (section 5.2)
- **DES-006** § DES-012 (Breath Between Chapters) — preserved (section 9)
- **DES-006** § DES-013 (Keyboard Navigation) — refined (section 13)
- **DES-025** (Accessibility) — reader-specific requirements consolidated (section 14)
- **DES-049** (Responsive) — reader breakpoints consolidated (section 15)
- **ADR-123** — Crimson color, font scales, timing values are named constants in `/lib/config.ts`
