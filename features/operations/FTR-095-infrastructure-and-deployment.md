...
All infrastructure is managed as code per SRF engineering standards. Vendor resources are managed by the yogananda-platform MCP server; one-time AWS security setup uses `bootstrap.sh`. See FTR-106 (revised 2026-03-01).
...
**API key scoping** (see FTR-094 § API Key Scoping Policy):
...
Values that vary per environment or contain secrets. The `.env.example` file is the local development reference. In deployed environments, the platform distributes secrets from AWS Secrets Manager (FTR-112) to Vercel environment variables via the Vercel API.
...
**Secrets management architecture (FTR-112).** Two tiers:
...
**AWS authentication by context (FTR-113):**
...
**Zero long-lived AWS credentials.** Vercel OIDC (FTR-113) authenticates Vercel functions to AWS via short-lived OIDC tokens. Combined with GitHub Actions OIDC, no long-lived AWS credentials exist in any environment.
...
#### 2. Named Constants (`/lib/config.ts` — FTR-012)

Tunable parameters that are not secrets and do not vary between environments. These are the "magic numbers" that FTR-012 requires as named constants. They change via code PRs, not env var updates.

```typescript
// /lib/config.ts — Named constants per FTR-012
// These are tunable defaults, not architectural commitments.
// Change via PR with evidence. Annotate: date, old → new, reason.

// ── AI Model Selection (FTR-105) ───────────────────────────────
...
**Operational surface companion:** FTR-096 specifies the operational *surface* built on top of this infrastructure foundation — health endpoint, SLI/SLO framework, and design-artifact traceability. Operational dashboarding is provided by the platform MCP server. FTR-095 is the infrastructure; FTR-096 is the visibility layer. See FTR-096, FTR-096, FTR-096, FTR-096.

## Notes

**Provenance:** FTR-095 → FTR-095
