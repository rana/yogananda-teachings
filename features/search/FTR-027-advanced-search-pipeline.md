...
**Two standard enhancements (M3a), one conditional (M3b+):**

**Milestone 3a (standard): Cross-Encoder Reranking**
...
**Milestone 3a (standard): Enrichment-Augmented Embeddings**
...
```
Primary mode (M1a-2b):  PATH A + PATH B -> CC(α=0.5) -> top 5
Enhanced mode (M3a):    PATH A + PATH B + HyDE -> CC(register-adaptive α) -> Voyage Rerank -> top 5
Full mode (M3b+):       PATH A + PATH B + PATH C + HyDE -> CC(three-path, register-adaptive) -> Voyage Rerank -> top 5
```

**Activation history:** Milestone 1a evaluation (92% Recall@3) confirmed pure hybrid search exceeds the 80% gate. Enhanced and full modes add precision and multi-dimensional relevance, not baseline adequacy.
...
- **Index-time enrichment bridges the vocabulary gap.** The Vocabulary Bridge, unified enrichment, and entity resolution map modern vocabulary to Yogananda's language at ingestion time. Enrichment-augmented embeddings (M3a) push this further: the enrichment metadata itself becomes searchable via vector similarity.
